...
HF_PUSH_EVERY=0  # v2: disabled. HF inline-payload + Android argv limit broke push past
                 # ~iter 530 in v1. Audit log stays local on /sdcard; adb-pulled in the morning.

# v2: low-duty-cycle "fridge, unplugged, on battery" mode
SLEEP_SECS=60   # was 10 in v1; ~6x lower NPU duty cycle, ~2x lower avg power draw
...
while true; do
  ...
  # HF push every N iterations (disabled in v2; HF_PUSH_EVERY=0)
  if [ "$HF_PUSH_EVERY" -gt 0 ] && [ $((iter % HF_PUSH_EVERY)) -eq 0 ]; then
    hf_push
  fi

  # Sleep between batches; v2 default 60s for low duty cycle (fridge-overnight battery mode)
  sleep "$SLEEP_SECS"
done
